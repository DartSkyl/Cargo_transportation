...
from keyboards import (main_executor, take_order, confirm_order_taking, cancel_button,
...
async def executor_taken_cargo(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete()
    await callback.message.answer('Скиньте фото, на котором видно весь груз!', reply_markup=cancel_button)
    await state.set_state(TakenOrder.add_photo)


@all_role_router.message(TakenOrder.add_photo, F.text != '⛔ Отмена')
@executor_router.message(TakenOrder.add_photo, F.text != '⛔ Отмена')
async def catch_executor_photo(msg: Message, state: FSMContext):
    ...
    if msg.photo:
        ...
        if msg.from_user.id in roles_dict['executor']:
